...
def show_chatbot_UI():
    ...
    # File uploader
    uploaded_file = st.file_uploader("랭체인 파이프라인 구축을 위한 파일을 선택해 주세요.", type=["pdf", "png", "jpg", "jpeg"])
    ...
    user_prompt = st.text_area("기본 시스템 프롬프트 (변경 가능):", value=st.session_state.system_prompt)
    if st.button("변경"):
